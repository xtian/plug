defmodule Plug.SessionTest do
  ...
  test "puts session cookie" do
    ...
    conn = conn(:get, "/") |> fetch_cookies
    opts = Plug.Session.init(store: ProcessStore, key: "foobar", secure: true, path: "some/path", extra_attrs: %{})
    conn = Plug.Session.call(conn, opts) |> fetch_session
    conn = put_session(conn, "foo", "bar")
    conn = send_resp(conn, 200, "")
    assert %{"foobar" => %{value: _, secure: true, path: "some/path", extra_attrs: %{}}} = conn.resp_cookies
